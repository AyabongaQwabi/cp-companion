 ...
 * auth token to forward, so a secret configured in both apps' env is the gate. Called cross-origin
 * from the browser (cp-redesign-admin is a separate deployment), so CORS headers are required —
 * see src/lib/admin-stats-cors.ts.
 */
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return withAdminStatsCors(NextResponse.json(result));
  } catch (err) {
    return withAdminStatsCors(
      NextResponse.json(
